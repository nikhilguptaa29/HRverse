...
class _ReportsPageState extends State<ReportsPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      attendanceProvider.fetchMonthsList();
      attendanceProvider.selectedMonth = null;
    });
  }
            ...
            children: [
              Container(
                ...
                child: Padding(
                  ...
                  child: DropdownButton<String>(
                    ...
                    items:
                        attendanceProvider.availableMonths
                            .map(
                              (mon) => DropdownMenuItem(
                                value: mon,
                                child: Text(
                                  DateFormat(
                                    'MMMM yyyy',
                                  ).format(DateFormat('yyyy-MM').parse(mon)),
                                ),
                              ),
                            )
                            .toList(),
                    onChanged:
                        (value) => attendanceProvider.selectMonth(value!),
                  ),
                ),
              ),
              // SizedBox(height: 20),
              attendanceProvider.selectedMonth == null
                  ? Padding(
                    ...
                    child: Center(
                      child: Text(
                        "Select the Month to view the Attendance",
                      ...
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          return Container(
                            ...
                            child: DataTable(
                              // dataRowColor: WidgetStatePropertyAll(Colors.grey.shade200),
                              headingRowColor: WidgetStatePropertyAll(
                                Colors.blue.shade100,
                              ),
                              columnSpacing: constraints.maxWidth / 25,
                              ...
                              rows:
                                  attendanceProvider.attendanceList
                                      .map(
                                        (attendance) => DataRow(
                                          cells: [
                                            DataCell(
                                            ...
                                            DataCell(
                                              SizedBox(
                                                width: width / 4.4,
                                                child: Text(
                                                  "${attendance.workHours.inHours} : ${attendance.workHours.inMinutes % 60}",
                                                ),
                                              ),
                                            ),
                                          ],
